Wordfly<- X17SeasonWordfly071817
#Rename loaded csv
head(Wordfly)
...
ggplot(Wordfly, aes(Time, UniqueOpenRate)) + geom_smooth() + labs(title ="17MKT UOR ToD SmoothLine by Weekday") + facet_wrap(~WeekDay)
...
ggplot(Opera, aes(Time, UniqueOpenRate)) + geom_smooth() + labs(title ="17WO ToD SmoothLine by Weekday") + facet_wrap(~WeekDay)
...
ggplot(Steph, aes(Time, UniqueOpenRate)) + geom_smooth() + labs(title ="17Steph ToD SmoothLine by Weekday") + facet_wrap(~WeekDay)
...
ggplot(Derek, aes(Time, UniqueOpenRate)) + geom_smooth() + labs(title ="17Derek ToD SmoothLine by Weekday") + facet_wrap(~WeekDay)
...
ggplot(Theater, aes(Time, UniqueOpenRate)) + geom_smooth() + labs(title ="17TT ToD SmoothLine by Weekday") + scale_y_continuous(limits=c(0,0.8)) + facet_wrap(~WeekDay) 
...
ggplot(Frank, aes(Time, UniqueOpenRate)) + geom_smooth() + labs(title ="17Frank ToD SmoothLine by Weekday") + scale_y_continuous(limits=c(0,0.8)) + facet_wrap(~WeekDay)
...
Doubletake<- subset(Wordfly, ECQ == 1)
...
table(Doubletake$Campaign)
Peek<- subset(Wordfly, ECQ ==2)
...
WNODouble<- subset(Opera, WNOC == 1)
WNOPeek<- subset(Opera, WNOC == 2)
...
DanceDouble<- subset(Dance, DMC == 4)
MixedDouble<-subset(Mixed, MDC == 1)
StephDouble<- subset(Steph, ADSC == 2)
StephPeek<- subset(Steph, ADSC == 5)
...
NSODouble<- subset(NSO, DBC == 1)
MixedDouble<-subset(Mixed, MDC == 1)
DerekDouble<- subset(Derek, DOGC == 2)
...
TheaterDouble<- subset(Theater, TTC == 3)
TheaterPeek<- subset(Theater, TTC == 5)
...
ComedyDouble<- subset(Comedy, ComC == 3)
JazzDouble<-subset(Jazz, JazzC == 3)
FrankDouble<- subset(Frank, FMC == 2)
FrankPeek<- subset(Frank, FMC ==5)
